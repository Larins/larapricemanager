...
//Insertions to DB on ARTICLES table.
	public void insertar_art(String art_nom, String art_ori, String art_date){
		try{
			//MySQL connectors.
			...
			consulta.executeUpdate("INSERT INTO articles (name, origin, date) VALUES ("
					+ "'" 
					+ art_nom 
					+ "'"
					+ ","
	...
	public void insertar_prom(String prom_nom, String prom_ori, String prom_date){
		try{
			//MySQL connectors.
			//DriverManager.registerDriver(new com.mysql.jdbc.Driver());
			//Connection parameters JDBC//Server//DB name>User>Password
			...
			consulta.executeUpdate("INSERT INTO promos (name, origin, date) VALUES ("
					+ "'" 
					+ prom_nom 
					+ "'"
					+ ","
...
public void selectPrices(String question){
	try{
		...
		Statement consulta = conexion.createStatement();
		//Article list query.
		ResultSet resultado = consulta.executeQuery("SELECT A.name, A.date FROM articles A INNER JOIN promos P ON P.date = A.date AND A.date = '" + question + "';");
		//Printing query result.
			//Title.
		...
		do{
			...
			switch(m){
				case 1://Add article.
					...
					String art_ori = JOptionPane.showInputDialog("Write an art_origin for the article");
					String art_date = JOptionPane.showInputDialog("Write a date for the article");
					insertar_art(art_nom, art_ori, art_date);
				...
				case 3://Add promotion.
					...
					String prom_ori = JOptionPane.showInputDialog("Write an art_origin for the promo");
					String prom_date = JOptionPane.showInputDialog("Write a date for the article");
					insertar_prom(prom_nom, prom_ori, prom_date);
